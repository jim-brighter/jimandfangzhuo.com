...
export class App {
  ...
  private initViewEvents() {
    ...
    this.imageGridView.addEventListener("modal-select", (event: Event) => {
      const customEvent = event as CustomEvent;
      const { imageUrl, optimizedUrl, imageAlt, videoUrl } = customEvent.detail;
      this.currentImageIndex = this.loadedImages.findIndex((img) => img.originalUrl === imageUrl);
      this.modalView.show(imageUrl, optimizedUrl, imageAlt, videoUrl);
    });
  ...
  private async navigateModal(direction: "next" | "prev") {
    ...
    if (direction === "next") {
      if (this.currentImageIndex < this.loadedImages.length - 1) {
        ...
        this.modalView.updateImage(nextImg.originalUrl, nextImg.optimizedUrl, "Album photo", nextImg.videoUrl);
      } else {
      ...
      if (this.currentImageIndex > 0) {
        ...
        this.modalView.updateImage(prevImg.originalUrl, prevImg.optimizedUrl, "Album photo", prevImg.videoUrl);
      } else {
