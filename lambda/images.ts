...
import { GetObjectCommand, ListObjectsV2Command, S3Client, ListObjectsV2CommandOutput } from '@aws-sdk/client-s3';
...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    if (albumId) {
      ...
      // 1. List objects from S3 with native pagination (MaxKeys: 20)
      ...
      const objects = (listRes.Contents || []).filter(o => o.Key && !o.Key.endsWith('/'));

      // 2. Generate signed URLs for page assets
      ...
      return response(200, {
        images: albumImages,
        nextPageToken: listRes.NextContinuationToken
      });
    }
